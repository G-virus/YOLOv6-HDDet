...
from yolov6.utils.v5_augmentations import random_perspective
from yolov6.utils.v5_rotation import box2rbox, rbox2box
...
class TrainValDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment and random.random() < self.hyp["mosaic"]:
            ...
            img, labels, shapes = self.get_general_obb(index) ### 这里面包括了常用aug
    ...
    def general_augment(self, img, labels):
        ...
        rbox = box2rbox(labels[:, 1:6])
        if random.random() < self.hyp["flipud"]:
            # img, labels = RFlipVertical(img, labels)
            img = np.flipud(img)
            if nl:
                rbox[:, 1::2] = img.shape[0] - rbox[:, 1::2]

        if random.random() < self.hyp["fliplr"]:
            # img, labels = RFlipHorizontal(img, labels)
            img = np.fliplr(img)
            if nl:
                rbox[:, 0::2] = img.shape[1] - rbox[:, 0::2]
        labels[:, 1:6] = rbox2box(rbox)
