...
from yolov6.utils.nms_R import (non_max_suppression_obb,
                                non_max_suppression_obb_cuda)
from yolov6.utils.torch_utils import get_model_info, time_sync
from yolov6.utils.v5_rotation import box2rbox, calc_HP
...
class Evaler:
    ...
    def predict_model(self, model, dataloader, task, epoch):
        """Model prediction
        ...
        pred_results = []
        nc = len(dataloader.dataset.class_names)
        ...
        if self.do_pr_metric:
            ...
            niou = iouv.numel()
            hccv = torch.linspace(0.001, 0.45, 10) # iou0 for  calc hcc
        ...
        for i, (imgs, targets, paths, shapes) in enumerate(pbar):
            ...
            # Statistics per image  pred [N, x, y, w, h, angle, conf, classes]
            # This code is based on  targets [imgid, cls, x, y, w, h, ang]
            ...
            for si, pred in enumerate(eval_outputs):
                ...
                if len(pred) == 0:
                    if nl:
                        stats.append((torch.zeros(0, niou, dtype=torch.bool), torch.Tensor(), torch.Tensor(), tcls, torch.zeros((nc, 2, 10), device='cpu')))
                    continue
                ...
                # Assign all predictions as incorrect  ### add nhcc
                correct = torch.zeros(pred.shape[0], niou, dtype=torch.bool)

                head_correct = torch.zeros((nc, 2, niou), dtype=torch.float)
                if nl:
                    # target boxes
                    tbox = labels[:, 1:5]
                    ...
                    from yolov6.utils.metrics_R import process_batch_rbox

                    # correct = process_batch(predn, labelsn, iouv) ### add hccv
                    correct, head_correct = process_batch_rbox(predn, labelsn, iouv, hccv, nc=nc)
                ...
                # current包括了 angle  这里只加hcc

                stats.append((correct.cpu(), pred[:, 5].cpu(), pred[:, 6].cpu(), tcls, head_correct.cpu()))

        if self.do_pr_metric:
            # Compute statistics
            stats = [np.concatenate(x, 0) for x in zip(*stats)]  # to numpy
            head_acc = np.zeros((1, nc))
            if len(stats) and stats[0].any():

                from yolov6.utils.metrics_R import ap_per_class

                p, r, ap, f1, ap_class, head_output = ap_per_class(
                    *stats, nc, plot=self.plot_curve, save_dir=self.save_dir, names=model.names, ap_method=self.ap_method
                )
                ...
                # head_output第一行是hcc 第二行是loss
                head_acc = head_output[:, 0, 0]
                head_loss = head_output[:, 1, 0]
                ...
                LOGGER.info(s)
                pf = "%-16s" + "%12i" * 2 + "%12.5g" * 7  # print format
                LOGGER.info(pf % ("all", seen, nt.sum(), mp, mr, f1.mean(0)[AP50_F1_max_idx], map50, map, head_acc.mean(), head_loss.mean()))

                self.pr_metric_result = (map50, map, head_acc.mean(), head_loss.mean())
    ...
    def eval_model(self, pred_results, model, dataloader, task): ### 默认不走,没改
        """Evaluate models
