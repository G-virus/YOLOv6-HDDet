...
from yolov6.utils.infer_utils import DetectMultiBackend
"""
...
class Evaler:
    ...
    def init_model_Deploy(self, model, weights, task):
        #NOTE Weights 对应不同推理框架的权重路径，
        if task!="train":
            #NOTE 和原始的推理框架相一致
            model = DetectMultiBackend(task=task, weights=weights, device=self.device, fp16=self.half)
            #NOTE 部分推理框架可能没
            # num_classes = Evaler.return_nc(self.data)
            model.nc = self.data["nc"]
            model.names = self.data["names"]
            self.stride = 32
            LOGGER.info("Deployment format has been intialized")
        return model
    ...
    def predict_model(self, model, dataloader, task):
        ...
        for i, (imgs, targets, paths, shapes) in enumerate(pbar):
            #NOTE FOR ONNX/TensorRT/Dynamic Inference
            nb_img = imgs.shape[0]
            ...
            # NOTE [N, x, y, w, h, angle, conf, classes],静态推理避免溢出的操作
            outputs = outputs[:nb_img]
            imgs = imgs[:nb_img]
            paths = paths[:nb_img]
        ...
        if self.do_pr_metric:
            ...
            if len(stats) and stats[0].any():
                ...
                p, r, ap, f1, ap_class = ap_per_class(
                    *stats, plot=self.plot_curve, save_dir=self.save_dir, names=model.names, ap_method=self.ap_method
                )
