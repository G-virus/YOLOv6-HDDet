...
class Detect(nn.Module):
    ...
    '''
    def __init__(self, num_classes=80, num_layers=3, inplace=True, head_layers=None, use_dfl=True, reg_max=16, angle_max = 180, angle_fitting_methods="regression"):  # detection layer
        super().__init__()
        assert head_layers is not None
        self.nc = num_classes  # number of classes
        self.no = num_classes + 6  # number of outputs per anchor
    ...
    def initialize_biases(self):
        ...
        if self.angle_fitting_methods == "dfl":
            ...
            self.proj_angle_conv.weight = nn.Parameter(
    ...
    def forward(self, x):
        if self.training:
            ...
            angle_reg_list = torch.cat(angle_reg_list, axis=1)
            angle_cls_list = torch.cat(angle_cls_list, axis=1)

            return x, cls_score_list, reg_distri_list, reg_lrtb_list, angle_cls_list, angle_reg_list
        else:
            cls_score_list = []
            reg_lrtb_list = []
            angle_reg_list = []
...
def build_effidehead_layer(channels_list, num_anchors, num_classes, reg_max=16, angle_fitting_methods="regression", angle_max=180):
    ...
    head_layers = nn.Sequential(
        ...
        nn.Conv2d(
            in_channels=channels_list[6],
            out_channels=angle_max * (num_anchors),
            kernel_size=1
        ),
        # stem1
        Conv(
