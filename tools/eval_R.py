...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--data", type=str, default="./data/DOTA.yaml", help="dataset.yaml path")
    parser.add_argument("--weights", type=str, default="./weights/best.pt", help="model.pt path(s)")
    ...
    parser.add_argument("--test_load_size", type=int, default=800, help="load img resize when test")
    parser.add_argument(
        "--letterbox_return_int", default=False, action="store_true", help="return int offset for letterbox"
    )
    parser.add_argument(
        "--scale_exact", default=False, action="store_true", help="use exact scale size to scale coords"
    )
    parser.add_argument("--force_no_pad", default=False, action="store_true", help="for no extra pad in letterbox")
    parser.add_argument(
        "--not_infer_on_rect", default=False, action="store_true", help="default to use rect image size to boost infer"
    )
    parser.add_argument(
        "--reproduce_640_eval",
        default=False,
...
def run(
    ...
    # eval
    model.eval()
    pred_result, vis_outputs, vis_paths = val.predict_model(model, dataloader, task, epoch)
